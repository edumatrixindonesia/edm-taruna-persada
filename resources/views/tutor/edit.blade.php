                ...
                <div class="card-body">
                    <form action="{{ route('tutor.update', $tutor['id']) }}" method="post">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="theme" class="form-control @error('theme') is-invalid @enderror" id="defaultFormControlInput" placeholder="Masukkan Tema" value="{{ $tutor['theme'] }}" />
                            @error('theme')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="mapel[]" id="defaultSelect" class="form-control mapel @error('mapel') is-invalid @enderror" multiple="multiple">
                                ...
                                <option value="{{ $mapel['name'] }}" @if(in_array($mapel['name'], $arr_mapel)) selected @endif>{{ $mapel['name'] }}</option>
                                @endforeach
                            </select>
                            @error('mapel')
                            <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="program[]" id="program" class="form-control program @error('program') is-invalid @enderror" multiple="multiple">
                                ...
                                <option value="{{ $program['namaProgram'] }}" @if(in_array($program['namaProgram'], $arr_program)) selected @endif>{{ $program['namaProgram'] }}</option>
                                @endforeach
                            </select>
                            @error('program')
                            <span class="invalid-feedback" role="alert">
